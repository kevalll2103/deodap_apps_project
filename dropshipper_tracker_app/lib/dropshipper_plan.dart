...
class PlanStep {
  ...
  final bool clientPending; // Added client_pending field
  final String? clientPendingNote; // Added client_pending_note field
  ...
  PlanStep({
    ...
    required this.clientPending, // Added to constructor
    this.clientPendingNote, // Added to constructor
  });
  ...
  factory PlanStep.fromJson(Map<String, dynamic> json) {
    return PlanStep(
      ...
      updatedAt: json['updated_at'] != null
          ...
          : DateTime.now(),
      clientPending: json['client_pending'] == 1 || json['client_pending'] == true || json['client_pending']?.toString().toLowerCase() == 'true', // Handle different data types
      clientPendingNote: json['client_pending_note'], // Parse client_pending_note
    );
  }
  ...
  bool get isClientPending => clientPending; // Added getter for client pending
  bool get hasClientPendingNote => clientPendingNote != null && clientPendingNote!.isNotEmpty; // Added helper getter
}
...
class _PlanScreenState extends State<PlanScreen> with WidgetsBindingObserver {
  ...
  Future<void> _checkForUpdates() async {
    try {
      // Here you would implement your actual update checking logic
      // For example, compare with the last known state or call an API
      final hasUpdates = await _checkForPlanUpdates();
      ...
      if (hasUpdates && mounted) {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('New updates available!'),
  ...
  Future<bool> _checkForPlanUpdates() async {
    // Implement your actual update checking logic here
    // For example, you could compare the last updated timestamp
    // with the server's timestamp or check for new steps
    final prefs = await SharedPreferences.getInstance();
    final lastUpdate = prefs.getString('last_plan_update');
    
    // Here you would typically make an API call to check for updates
  ...
  Future<void> _openStepChat(PlanStep step) async {
    ...
    if (updatedStatus != null && updatedStatus != step.status) {
      // Update the step status in the list
      final updatedSteps = planDetails!.steps.map((s) {
        if (s.stepId == step.stepId) {
          return PlanStep(
            ...
            clientPending: s.clientPending, // Include client_pending
            clientPendingNote: s.clientPendingNote, // Include client_pending_note
          );
        }
        return s;
      }).toList();
  ...
  Widget _buildStepsList() {
    ...
    final clientPendingSteps = steps.where((step) => step.isClientPending).length; // Added client pending count
          ...
          child: Container(
            ...
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 16),
                // Status chips - Added client pending
                Wrap(
                  spacing: 8,
                  children: [
                    ...
                    if (clientPendingSteps > 0) // Show client pending chip only if there are client pending steps
                      _buildStatusChip('Client Pending', clientPendingSteps, Colors.purple),
                  ],
                ),
              ],
            ),
  ...
  Widget _buildStepCard(PlanStep step, int index) {
    ...
    // Updated status handling to include client pending
    if (step.isCompleted) {
    ...
    } else if (step.isClientPending) { // Check client pending before general pending
      statusColor = Colors.purple;
      statusIcon = Icons.person_pin_circle_rounded;
      statusText = 'Client Pending';
    } else {
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      // Show client pending note if available
                      if (step.isClientPending && step.hasClientPendingNote) ...[
                        const SizedBox(height: 8),
                        Container(
                          padding: const EdgeInsets.all(8),
                          decoration: BoxDecoration(
