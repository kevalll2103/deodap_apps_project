...
import 'package:dropshipper_tracker_app/dashboard.dart';

class Login extends StatefulWidget {
  const Login({super.key});

  @override
  State<Login> createState() => _LoginScreenAdminViewState();
}

class _LoginScreenAdminViewState extends State<Login>
  ...
  // Keys used in SplashscreenView - UPDATED KEYS
  static const String KEY_LOGIN = 'isLoggedIn';
  static const String KEY_LOGIN_TYPE ='loginType'; // Changed from KEY_USER_ROLE
  ...
  Future<void> _checkAutoLogin() async {
    ...
    final String? loginType =
        prefs.getString(KEY_LOGIN_TYPE); // Updated to use loginType

    // Check करें अगर user पहले से login है और type 'user' है
  ...
  Future<void> _login() async {
    ...
    final url = Uri.parse(
        'https://customprint.deodap.com/api_dropshipper_tracker/login_dropshipper.php');
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (data['success'] == true) {
          ...
          // Save session info - UPDATED TO USE LOGIN TYPE
          await prefs.setBool(KEY_LOGIN, true);
          await prefs.setString(
              KEY_LOGIN_TYPE, 'user'); // Changed from 'employee' to 'user'
          await prefs.setString('user_id', user['id'].toString());
          await prefs.setString('seller_id', user['seller_id'] ?? '');
          await prefs.setString('seller_name', user['seller_name'] ?? '');
          await prefs.setString('store_name', user['store_name'] ?? '');
          await prefs.setString('contact_number', user['contact_number'] ?? '');
          await prefs.setString('email', user['email'] ?? '');
          await prefs.setString('crn', user['crn'] ?? '');
          await prefs.setString('username', user['username'] ?? '');
          await prefs.setString('admin_password', password);

          // Save user data as JSON for dashboard
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 8),
            Text(
              "Sign in to your dropshipper account",
              textAlign: TextAlign.center,
              style: GoogleFonts.inter(
                fontSize: 14,
                fontWeight: FontWeight.w400,
  ...
  Widget _buildCheckboxSection() {
    return Column(
      children: [
        _buildCheckbox("Remember me", rememberMe,
              ...
              children: [
                WidgetSpan(
                  child: GestureDetector(
                    onTap: () =>
                        _launchURL('https://deodap.in/pages/terms-conditions'),
                    child: Text(
                      "Terms & Conditions",
                      style: GoogleFonts.inter(
                        color: primaryBlue,
