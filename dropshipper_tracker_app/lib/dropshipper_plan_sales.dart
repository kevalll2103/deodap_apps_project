...
class PlanStep {
  ...
  final String? clientPendingNote;
  ...
  factory PlanStep.fromJson(Map<String, dynamic> json) {
    return PlanStep(
      ...
      updatedAt: json['updated_at'] != null
          ...
          : DateTime.now(),
      clientPending: json['client_pending'] == 1 || json['client_pending'] == true || json['client_pending']?.toString().toLowerCase() == 'true',
      clientPendingNote: json['client_pending_note'],
    );
  }
  ...
  bool get isClientPending => clientPending;
  bool get hasClientPendingNote => clientPendingNote != null && clientPendingNote!.isNotEmpty;
}
...
class PlanScreen_sales extends StatefulWidget {
  final int? planId;
  final String? planName;
  final int? dropshipperId;

  const PlanScreen_sales({
    super.key,
    this.planId,
    this.planName,
    this.dropshipperId,
  ...
  @override
  State<PlanScreen_sales> createState() => _PlanScreenState();
}

class _PlanScreenState extends State<PlanScreen_sales> with WidgetsBindingObserver {
  ...
  Widget _buildStepsList() {
    ...
    final clientPendingSteps = steps.where((step) => step.isClientPending).length;
