...
class _LoginScreenState extends State<LoginScreen> {
  ...
  // --- API Config ---
  static const String baseUrl = 'https://api.vacalvers.com/api-packaging-app';
  static const String appId = '1';
  static const String apiKey = 'c77b74df-59f6-4257-b0ee-9b81e30026b1';
  ...
  Future<void> _saveLoginSession(Map<String, dynamic> res) async {
    ...
    await p.setInt(
      'warehouseId',
      (user['warehouse_id'] ?? 0) is int
          ? user['warehouse_id']
          : int.tryParse('${user['warehouse_id']}') ?? 0,
    );
    await p.setInt(
      'isReadonly',
      (user['is_readonly'] ?? 0) is int
          ? user['is_readonly']
          : int.tryParse('${user['is_readonly']}') ?? 0,
    );
  ...
  void _welcomeAutoGo() {
    ...
    Future.delayed(const Duration(seconds: 3), () {
      ...
      Get.offAllNamed(AppRoutes.home); // go to Home via GetX
    });
  }
