...
$apiUrl = "https://customprint.deodap.com/api_dropshipper_tracker/emp_details.php"; // change path accordingly

// Build query parameters with improved handling
$queryParams = [];

// Add emp_code parameter if specified (with validation)
if (isset($_GET['emp_code']) && !empty($_GET['emp_code'])) {
    $emp_code = filter_var($_GET['emp_code'], FILTER_SANITIZE_STRING);
    if (!empty($emp_code)) {
        $queryParams['emp_code'] = $emp_code;
    }
}
...
if (isset($_GET['sort_by']) && !empty($_GET['sort_by'])) {
    ...
    $allowed_sort_fields = ['id', 'emp_name', 'emp_code', 'created_at', 'email'];
...
// Handle data structure: backend returns 'data' as array for all employees or object for single
if (isset($data['data'])) {
...
// Filter by emp_code if provided
if (isset($_GET['emp_code']) && !empty($_GET['emp_code'])) {
    $filter_emp_code = filter_var($_GET['emp_code'], FILTER_SANITIZE_STRING);
    $employees = array_filter($employees, function($e) use ($filter_emp_code) {
        return isset($e['emp_code']) && $e['emp_code'] == $filter_emp_code;
    });
}
?>
<!DOCTYPE html>
...
<title>Employee Details - Admin Portal</title>
...
<div class="sidebar" id="sidebar" style="height: 100vh; width: 280px; position: fixed; top: 0; left: 0; background: #ffffff; box-shadow: 2px 0 15px rgba(0,0,0,0.08); z-index: 1000; overflow-y: auto; border-right: 1px solid #e5e7eb;">
    <div style="padding: 30px 20px; border-bottom: 1px solid #e5e7eb; background: #6366f1;">
        <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 8px;">
            ...
            <div>
                ...
                <p style="color: rgba(255,255,255,0.8); font-size: 14px; margin: 0; font-weight: 400;">Employee Management</p>
            </div>
        </div>
    </div>
 ...
 <div style="padding: 8px 20px; font-size: 12px; font-weight: 600; color: #6b7280; text-transform: uppercase; letter-spacing: 0.5px; margin-bottom: 4px;">EMPLOYEES</div>
    ...
    <div class="header-content">
        <div class="header-left">
            ...
            <div class="header-text">
                ...
                <p>Monitor and manage your registered employees</p>
            </div>
        </div>
...
<?php echo ($data && $data['success'] && !empty($employees)) ? count($employees) : '0'; ?>
</span>
...
<span class="stat-number">
<?php
if ($data && $data['success'] && !empty($employees)) {
echo count(array_filter($employees, function($e) {
return !empty($e['created_at']) && strtotime($e['created_at']) > strtotime('-30 days');
}));
} else {
echo '0';
}
...
<?php if ($data && $data['success'] && !empty($employees)): ?>
...
<th><i class="fas fa-id-card"></i> Employee Code</th>
...
<td>
<div class="id-badge">
<strong><?= htmlspecialchars($e['emp_id']); ?></strong>
</div>
</td>
<td>
<div class="seller-info">
<div class="seller-avatar">
<?= strtoupper(substr(htmlspecialchars($e['emp_name']), 0, 1)); ?>
</div>
<div class="seller-details">
<h4><?= htmlspecialchars($e['emp_name']); ?></h4>
<p>emp ID: <?= htmlspecialchars($e['emp_id']); ?></p>
</div>
</div>
</td>
<td>
<strong><?= htmlspecialchars($e['emp_dept'] ?? 'seller support'); ?></strong>
</td>
...
<?= htmlspecialchars($e['contact_number'] ?? 'N/A'); ?>
</div>
<div class="contact-item">
...
<td>
<code><?= htmlspecialchars($e['emp_code']); ?></code>
</td>
<td>
<div class="contact-info">
<div class="contact-item">
<i class="fas fa-user"></i>
<?= htmlspecialchars($e['username'] ?? 'N/A'); ?>
</div>
<div class="contact-item password-field">
<i class="fas fa-lock"></i>
<span class="password-hidden">••••••••</span>
<i class="fas fa-eye password-toggle" onclick="togglePassword(this, '<?= htmlspecialchars($e['plain_password'] ?? 'password'); ?>')"></i>
</div>
</div>
</td>
<td>
<div class="date-badge">
<?= date('M d, Y', strtotime(htmlspecialchars($e['created_at']))); ?>
</div>
</td>
<td>
...
<td>
<button class="btn-plan" onclick="viewDropshipperPlans('<?= htmlspecialchars($e['emp_id']); ?>')">
...
<p>There are currently no registered employees in the system.</p>
</div>
<?php endif; ?>
</div>
</div>
...
function viewDropshipperPlans(empid) {
    // Navigate to the dropshipper_wise_plan.php page with the emp ID in the same tab
    window.location.href = `employee_wise_plan.php?emp_id=${empid}`;
}
</script>
</body>
</html>
