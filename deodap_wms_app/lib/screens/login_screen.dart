...
class _LoginScreenState extends State<LoginScreen> {
  ...
  // --- Theme bits ---
  // Brand Purple as iOS "active" color
  static const Color _brand = Color(0xFF6B52A3);
  Color get _iosBlue => _brand;
  ...
  Future<void> _saveLoginSession(Map<String, dynamic> res) async {
    ...
    await p.setInt('userId', (user['id'] ?? 0) is int ? user['id'] : int.tryParse('${user['id']}') ?? 0);
    await p.setString('userName', '${user['name'] ?? ''}');
    await p.setString('userPhone', '${user['phone'] ?? ''}');
    await p.setInt('warehouseId', (user['warehouse_id'] ?? 0) is int ? user['warehouse_id'] : int.tryParse('${user['warehouse_id']}') ?? 0);
    await p.setInt('isReadonly', (user['is_readonly'] ?? 0) is int ? user['is_readonly'] : int.tryParse('${user['is_readonly']}') ?? 0);
  ...
  /// SUCCESS dialog (no buttons) -> auto close + navigate after 3 seconds.
  void _welcomeAutoGo() {
    if (!mounted || _welcomeNavigated) return;
    ...
    showCupertinoDialog(
      ...
      builder: (_) => const CupertinoAlertDialog(
        ...
        content: Padding(
          ...
          child: Text('DeoDap employee • WMS Management System'),
        ),
      ),
    );
  ...
  Widget build(BuildContext context) {
    ...
    final logoHeight = (isTablet ? 140 : 120).toDouble(); // ↑ bigger login.png
                    ...
                    child: Form(
                      ...
                      child: Column(
                        children: [
                          ...
                          // Login button — iOS look, with icon
                          SizedBox(
...
class _CupertinoListTile extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    return Material( // subtle tap feedback inside popup
      color: Colors.white,
      child: InkWell(
        onTap: onTap,
        child: Container(
            ...
            children: [
              ...
              AnimatedSwitcher(
                ...
                child: selected
                    ...
                    color: Color(0xFF34C759), key: ValueKey('sel'))
