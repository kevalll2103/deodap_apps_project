...
import 'user_details.dart'; // for Employee model

class myPickedOrdersScreen extends StatefulWidget {
  const myPickedOrdersScreen({super.key});

  @override
  State<myPickedOrdersScreen> createState() =>
      _myPickedOrdersScreenState();
}

class _myPickedOrdersScreenState
    extends State<myPickedOrdersScreen> {
  // ====== CONFIG ======
  static const String _base = 'https://customprint.deodap.com/stockbridge';
  static const String _endpoint = '$_base/picked_order.php';
  ...
  // Employee loaded from SharedPreferences
  Employee? _employee;
  ...
  /// Load employee details from SharedPreferences
  Future<void> _loadEmployeeAndFetch() async {
    ...
    try {
      ...
      // Saved in login _saveSession(...)
      final idStr = p.getString('employee_id') ?? p.getInt('user_id')?.toString();
      final name = p.getString('name') ?? '';
      final empCode = p.getString('employee_emp_code') ?? '';
      final phone = p.getString('contact_number') ?? '';

      if (idStr == null || idStr.isEmpty) {
        setState(() {
          _empLoading = false;
          _empError =
              'Employee details not found. Please login again.';
        });
        return;
      }

      final empId = int.tryParse(idStr) ?? 0;

      // Build Employee model from prefs
      // Using default values for required fields that aren't available in shared prefs
      final emp = Employee(
        id: empId,
        name: name.isEmpty ? 'Employee' : name,
        empCode: empCode.isEmpty ? 'N/A' : empCode,
        email: '',  // Not available in shared prefs
        username: name.toLowerCase().replaceAll(' ', '_'),  // Generate from name
        contactNumber: phone,
        isActive: true,  // Default to active
        lastLoginIp: '0.0.0.0',  // Default value
        lastLoginTime: DateTime.now().toIso8601String(),  // Current time
        createdAt: DateTime.now().toIso8601String(),  // Current time
        updatedAt: DateTime.now().toIso8601String(),  // Current time
      );
      ...
      // Now fetch picked orders for this employee
      await _fetchData();
    } catch (e) {
      setState(() {
        _empLoading = false;
        _empError = 'Error loading employee details: $e';
      });
    }
  }
  ...
  Future<void> _fetchData() async {
    ...
    if (emp == null) {
      setState(() {
        _loading = false;
        _error = 'Employee not loaded. Please login again.';
      });
      return;
    }
    ...
    try {
      ...
      // 1) If non-200, do not try to parse JSON; show raw body snippet.
      ...
      final empNode = emps.first;
      final totalOrdersNum = empNode['total_orders_picked'];
      final totalOrders = (totalOrdersNum is num)
          ? totalOrdersNum.toInt()
          : int.tryParse('$totalOrdersNum') ??
              (empNode['orders'] is List
                  ? (empNode['orders'] as List).length
                  : 0);

      final ordersList =
          List<Map<String, dynamic>>.from(empNode['orders'] ?? []);
      final orders = ordersList.map((o) => PickedOrder.fromJson(o)).toList();
      final int totalQtyAll = orders.fold(
        0,
  ...
  void _openOrderDetails(PickedOrder order) {
    ...
    if (_empLoading) {
      return const CupertinoPageScaffold(
        navigationBar: CupertinoNavigationBar(
          middle: Text('Picked Orders'),
        ),
        child: Center(child: CupertinoActivityIndicator()),
      );
    }

    if (_empError != null || _employee == null) {
      return CupertinoPageScaffold(
        ...
        child: Center(
          child: Padding(
            ...
            child: Text(
              _empError ?? 'Employee details not available.',
  ...
  Widget _buildHeaderSummaryCard(Employee emp) {
    return Padding(
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    'From ${_dispFmt.format(_from)} to ${_dispFmt.format(_to)}',
                    style: GoogleFonts.inter(
                      fontSize: 11,
  ...
  Widget _buildBody() {
    ...
    if (_orders.isEmpty) {
      return Center(
        child: Text(
          'No picked orders in this range.',
          style: GoogleFonts.inter(
            fontSize: 15,
            color: CupertinoColors.secondaryLabel
                .resolveFrom(context),
    ...
    return CupertinoScrollbar(
      child: ListView.builder(
        ...
        itemBuilder: (_, i) => GestureDetector(
          onTap: () => _openOrderDetails(_orders[i]),
...
// ====== UI WIDGETS: Date chip & order list card ======
...
class _OrderCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final int totalQty =
        order.items.fold(0, (sum, it) => sum + (it.qty)); // total item qty
    ...
    final topRight = [
      if (order.channel != null && order.channel!.isNotEmpty)
        order.channel,
...
// ====== ORDER DETAIL SCREEN: show ALL order & item fields ======
...
class OrderDetailScreen extends StatelessWidget {
  ...
  int get _totalQty =>
      order.items.fold(0, (sum, it) => sum + (it.qty)); // for summary card
          ...
          children: [
            ...
            _Section(
              ...
              children: [
                ...
                _info('Picked By Emp ID',
                    order.pickedByEmpId?.toString()),
        ...
        children: [
          ...
          if (item.sellingPricePerItem != null)
            _InfoRow(
              ...
              value:
                  item.sellingPricePerItem!.toStringAsFixed(2),
            ),
          if (item.shippingChargePerItem != null)
            _InfoRow(
              label: 'Shipping / item',
              value: item.shippingChargePerItem!
                  .toStringAsFixed(2),
            ),
