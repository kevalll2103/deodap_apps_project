...
class EmployeePickedOrdersScreen extends StatefulWidget {
  final Employee employee;
  const EmployeePickedOrdersScreen({super.key, required this.employee, required String userId});

  @override
  State<EmployeePickedOrdersScreen> createState() =>
      _EmployeePickedOrdersScreenState();
}

class _EmployeePickedOrdersScreenState
    extends State<EmployeePickedOrdersScreen> {
  // ====== CONFIG ======
  ...
  void initState() {
    super.initState();
    _fetchData(); // default today -> today
  }
  ...
  Future<void> _fetchData() async {
    ...
    try {
      ...
      final params = {
        'emp_id': widget.employee.id.toString(),
      ...
      final int totalQtyAll = orders.fold(
        0,
        (sum, order) =>
            sum + order.items.fold(0, (s, it) => s + (it.qty)),
      );
