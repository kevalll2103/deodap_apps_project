...
class InwardSkuWarehouseStaff extends StatefulWidget {
  const InwardSkuWarehouseStaff({super.key});

  @override
  State<InwardSkuWarehouseStaff> createState() =>
      _InwardSkuWarehouseStaffState();
}

class _InwardSkuWarehouseStaffState extends State<InwardSkuWarehouseStaff> {
  // Controllers
  ...
  // Admin gate for base-image upload
  bool _baseUploadUnlocked = false;

  // Rack stock edit state
  final Map<int, String> _rackInStockEdited = {};
  bool isUpdatingRackStock = false;

  // NEW: Search list state (for manual search)
  ...
  void _resetScreen() {
    setState(() {
      ...
      _rackInStockEdited.clear();
      _baseUploadUnlocked = false;
  ...
  /// Scanner behaviour: scan → directly load that single SKU
  Future<void> fetchStockDetails(String skuCode) async {
    ...
    try {
      ...
      debugPrint(
          'Stock API (single) status: ${response.statusCode}, body: ${response.body}');
  ...
  /// Manual search behaviour: text search → list of all matching SKUs
  Future<void> _searchSkuList(String query) async {
    ...
    try {
      ...
      debugPrint(
          'Stock API (search list) status: ${response.statusCode}, body: ${response.body}');
      ...
      if (response.statusCode == 200) {
        ...
        if (decoded is Map<String, dynamic> &&
          ...
          final List<Map<String, dynamic>> list = (decoded['data'] as List)
          ...
          setState(() {
            _searchResults = list;
            stockData = null; // show list first
            rackStockData = null;
            baseProductImages = [];
            dailyProductImages = [];
            newBaseImages.clear();
        ...
        _showErrorDialog(
            'Failed to search stock (HTTP ${response.statusCode}).');
      }
    } catch (e) {
      overlayEntry.remove();
      if (e.toString().contains('timeout')) {
        ...
        _showErrorDialog('Error while searching stock.');
      }
    }
  }
  ...
  // Small helper: set stockData + images from a single record
  void _setStockFromRecord(Map<String, dynamic> first) {
    ...
    final String? skuId = stockData?['sku_id']?.toString();
  ...
  Future<void> _updateOmsRackStock() async {
    ...
    for (int i = 0; i < rackStockData!.length; i++) {
      ...
      final edited =
          _rackInStockEdited[i] ?? rack['in_stock']?.toString() ?? '0';
      final inStock = int.tryParse(edited) ?? 0;
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (body['error'] == 0) {
          _showSuccessDialog('Rack stock updated successfully in OMSGuru.');
          final skuId = stockData!['sku_id'].toString();
          fetchOmsRackStock(skuId);
        } else {
          _showErrorDialog(
  ...
  // ===== ADMIN GATE FOR BASE IMAGES =====

  Future<bool> _ensureBaseUploadPermission() async {
    if (_baseUploadUnlocked) return true;

    final TextEditingController passCtrl = TextEditingController();

    final String? entered = await showCupertinoDialog<String>(
      context: context,
      builder: (context) => CupertinoAlertDialog(
        ...
        content: Padding(
          ...
          child: Column(
            ...
            children: [
              const Text(
                'Only admin can upload base images.\nEnter admin password to continue.',
              ),
        ...
        actions: [
          ...
          CupertinoDialogAction(
            ...
            onPressed: () =>
                Navigator.of(context).pop(passCtrl.text.trim()),
    ...
    if (entered == 'admin@deodap7070') {
      setState(() {
        _baseUploadUnlocked = true;
      });
      await _showAutoDismissDialog(
        title: 'Admin Unlocked',
        message: 'You can now upload base images in this session.',
        titleColor: CupertinoColors.activeGreen,
      );
      return true;
    } else {
      _showErrorDialog('Incorrect admin password.');
      return false;
    }
  }
  ...
  Future<void> _uploadProductImages(ProductImageType type) async {
    ...
    if (type == ProductImageType.base) {
      final allowed = await _ensureBaseUploadPermission();
      if (!allowed) return;
    }
        ...
        children: [
          ...
          Row(
            children: [
              ...
              Expanded(
                  child:
                      _buildReturnConditionButton("Bad", 'bad', Colors.red)),
              const SizedBox(width: 10),
              Expanded(
                  child: _buildReturnConditionButton(
                ...
                return Container(
                  ...
                  child: Stack(
                    children: [
                      GestureDetector(
                        onTap: () =>
                            _openImageViewer(bytes: photos[index]),
                        child: ClipRRect(
                          borderRadius: BorderRadius.circular(12),
                          child: Image.memory(
                            photos[index],
  ...
  Widget _buildTopSearchRow() {
    return Row(
      children: [
        // Scanner → directly load single SKU
        CupertinoButton(
          ...
          onPressed: () async {
            ...
            if (scannedSkuCode != null) {
              ...
              setState(() {
                _searchResults.clear(); // hide list
              });
        ...
        const SizedBox(width: 8),
        // Manual search → show list of SKUs
        Expanded(
          child: Container(
            decoration: BoxDecoration(
  ...
  // =========== SEARCH RESULTS LIST (manual search) ===========

  void _onSkuResultTap(Map<String, dynamic> item) {
    FocusScope.of(context).unfocus();
    _setStockFromRecord(Map<String, dynamic>.from(item));
    setState(() {
      _searchResults.clear(); // same as other screen – hide list after select
    });
  }
        ...
        children: [
          ...
          ListView.separated(
            ...
            itemBuilder: (context, index) {
              ...
              final sku = item['sku_code']?.toString() ?? 'N/A';
              final name = item['name']?.toString() ?? 'No name';
              final inStock = item['in_stock']?.toString() ?? '0';
              final whId = item['warehouse_id']?.toString() ?? '';

              return InkWell(
                borderRadius: BorderRadius.circular(12),
                onTap: () => _onSkuResultTap(item),
                child: Padding(
                  padding:
  ...
  Widget _buildImageListSection(ProductImageType type) {
    ...
    final List<Uint8List> newOnes =
        isBase ? newBaseImages : newDailyImages;

    final bool isUploadingLocal =
        isBase ? isUploadingBaseImages : isUploadingDailyImages;

    final int maxBase = _maxBaseImages;
    final int totalBaseCount =
        baseProductImages.length + newBaseImages.length;

    final bool canAddMore =
        isBase ? totalBaseCount < maxBase : true;
    ...
    return Column(
      ...
      children: [
                  ...
                  child: Stack(
                    children: [
                      ...
                      Positioned(
                        ...
                        child: GestureDetector(
                          onTap: () =>
                              _confirmDeleteProductImage(type, url),
                          child: Container(
                            padding: const EdgeInsets.all(4),
                            decoration: const BoxDecoration(
                              color: Colors.red,
                ...
                return Container(
                  ...
                  child: Stack(
                    children: [
                      GestureDetector(
                        onTap: () =>
                            _openImageViewer(bytes: newOnes[index]),
                        child: ClipRRect(
                          borderRadius: BorderRadius.circular(12),
                          child: Image.memory(
                            newOnes[index],
            ...
            Expanded(
              child: CupertinoButton(
                ...
                color: canAddMore
                    ...
                    : CupertinoColors.systemGrey3,
                borderRadius: BorderRadius.circular(12),
                onPressed: (!canAddMore || isUploadingLocal)
                    ? null
                    : () async {
                        if (isBase) {
                          final allowed =
                              await _ensureBaseUploadPermission();
                          if (!allowed) return;
                        }
        ...
        if (isBase) ...[
          ...
          Text(
            'Maximum $_maxBaseImages base images allowed. Only admin can upload/replace base images.',
            style: GoogleFonts.poppins(
              fontSize: 11,
              color: const Color(0xFF9CA3AF),
            ),
  ...
  // ---------- Rack-wise section with added-stock preview ----------

  Map<String, int> _calculateRackTotals() {
    int originalTotal = 0;
    int newTotal = 0;

    if (rackStockData == null) {
      return {'original': 0, 'new': 0, 'delta': 0};
    }
    ...
    for (int i = 0; i < rackStockData!.length; i++) {
      ...
      final String editedStr =
          _rackInStockEdited[i] ?? rack['in_stock']?.toString() ?? '0';
      final int edited = int.tryParse(editedStr) ?? original;
      newTotal += edited;
    }

    final int delta = newTotal - originalTotal;
    return {'original': originalTotal, 'new': newTotal, 'delta': delta};
  }
      ...
      child: Column(
        ...
        children: [
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      'Per rack in-stock / bad-stock (editable)',
                      style: GoogleFonts.poppins(
                        fontSize: 13,
                        color: const Color(0xFF059669),
                        fontWeight: FontWeight.w500,
          ...
          if (isRackLoading)
            ...
            ListView.separated(
              ...
              itemBuilder: (context, index) {
                ...
                final String originalStr =
                    rack['in_stock']?.toString() ?? '0';
                final int original =
                    int.tryParse(originalStr) ?? 0;

                final String currentText =
                    _rackInStockEdited[index] ?? originalStr;
                final TextEditingController ctrl =
                    TextEditingController(text: currentText);

                final int edited =
                    int.tryParse(currentText) ?? original;
                final int diff = edited - original;
                        ...
                        children: [
                          SizedBox(
                            ...
                            child: TextField(
                              ...
                              onChanged: (value) {
                                setState(() {
                                  _rackInStockEdited[index] = value;
                                });
                              },
                            ),
                          ),
            ...
            Builder(
              builder: (_) {
                final totals = _calculateRackTotals();
                final original = totals['original']!;
                final newTotal = totals['new']!;
                ...
                if (delta > 0) {
                  ...
                  deltaText = '+$delta added';
                } else if (delta < 0) {
                  deltaColor = const Color(0xFFDC2626);
                  deltaText = '$delta reduced';
                } else {
                  deltaColor = const Color(0xFF6B7280);
                  deltaText = 'No change';
                }
